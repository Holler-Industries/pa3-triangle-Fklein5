...
public class Triangle {
    ...
    public double perimeter() {
        double sideA = Math.sqrt((x1 - x0) * (x1 - x0) + (y1 - y0) * (y1 - y0));


        double sideB = Math.sqrt((x2 - x0) * (x2 - x0) + (y2 - y0) * (y2 - y0));


        double sideC = Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
    ...
    public void tranlsateX(double Xchange){
